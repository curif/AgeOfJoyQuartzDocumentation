...
- `CabDBGetInfo(cabinetName, path)`: reads a single field from a cabinet's `description.yaml`, given its DB name and a field `path`. The path is case-insensitive and can reach nested fields with `.` and list items with `[n]` or `.n`. Examples: `CabDBGetInfo("pacman", "year")`, `CabDBGetInfo("pacman", "crt.type")`, `CabDBGetInfo("pacman", "parts[3].art.file")`. Fails with a runtime error if the cabinet doesn't exist, the field path doesn't match anything, or the path points to a list/group of fields instead of a single value (in that case, add an index or a further field name to reach a concrete value). Note: `description.yaml` doesn't currently have an `author` or `description` field, so those aren't available through this function yet — only fields that already exist in the yaml (`name`, `year`, `style`, `core`, `crt.*`, `model.*`, `parts[n].*`, etc.) can be read this way.
